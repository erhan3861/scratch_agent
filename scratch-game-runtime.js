(() => {
    ...
    /**
     * Scratch 3 inline input tipleri:
     * 4  = number
     * 5  = positive number
     * 6  = positive integer
     * 7  = integer
    ...
    function reporterInput(blocks, parentId, opcode, shadowValue = [4, "0"]) {
        ...
        return [3, reporterId, shadowValue];
    }

    function normalizeTouchingTarget(target) {
    ...
    function ensureDependencies() {
        ...
        if (!dependencyPromise) {
            dependencyPromise = loadScript(CONFIG.jszipUrl);
        }
    ...
    function ensureLayout() {
        ...
        if (!document.getElementById("scratch-game-root")) {
            ...
            root.innerHTML = `
                ...
                <iframe 
                    id="scratch-game-frame" 
                    title="Sheeptester Scratch Editörü"
    ...
    function readEmbeddedGame() {
        ...
        if (!source) {
            throw new Error("#game-code bulunamadı.");
        }

        try {
            return JSON.parse(source.textContent);
        } catch (error) {
            throw new Error(`JSON okunamadı: ${error.message}`);
        }
    }

    function compileCondition(definition, blocks, parentBlockId) {
        if (!definition || !definition.komut) return null;

        if (definition.komut === "algilama_degiyormu") {
            const reporterId = newBlockId();
            const menuId = newBlockId();
            const target = normalizeTouchingTarget(definition.hedef);
        ...
        throw new Error(`Desteklenmeyen şart komutu: ${definition.komut}`);
    }
    ...
    function compileBlockList(list, blocks, parentId) {
        ...
        (list || []).forEach((definition, index) => {
            if (!definition || !definition.komut) {
                throw new Error("Blok tanımı eksik veya hatalı.");
            }

            const opcode = COMMANDS[definition.komut];

            if (!opcode) {
                throw new Error(`Desteklenmeyen komut: ${definition.komut}`);
            }
            ...
            const block = {
                opcode,
                next: null,
                parent: index === 0 ? parentId : previousId,
                inputs: {},
                fields: {},
                shadow: false,
                topLevel: false
            ...
            if (definition.sart) {
                block.inputs.CONDITION = compileCondition(definition.sart, blocks, id);
            }
            ...
            if (definition.alt_bloklar) {
                ...
                if (substackId) {
                    block.inputs.SUBSTACK = [2, substackId];
                }
            }

            /**
             * Scratch'te control_stop ve control_forever blokları stack sonu gibi davranır.
             * Kullanıcı yanlışlıkla ardından blok yazarsa GUI'nin bozulmaması için next'i koparıyoruz.
             */
            if (
                definition.komut === "kontrol_durdur" ||
                definition.komut === "kontrol_surekli"
            ) {
                previousId = null;
            }
        });
    ...
    function createStageTarget(stageColor) {
        return {
            isStage: true,
            name: "Stage",
            variables: {},
            ...
            costumes: [
                {
                    assetId: CONFIG.stageAssetId,
                    name: "Arka Plan",
                    md5ext: `${CONFIG.stageAssetId}.svg`,
                    dataFormat: "svg",
                    bitmapResolution: 1,
                    rotationCenterX: 240,
                    rotationCenterY: 180
    ...
    function compileGame(model) {
        ...
        assets.set(
            ...
            `<svg xmlns="http://www.w3.org/2000/svg" width="480" height="360" viewBox="0 0 480 360"><rect width="480" height="360" fill="${stageColor}"/></svg>`
        );

        const targets = [createStageTarget(stageColor)];

        model.karakterler.forEach((character, index) => {
            if (!character || !character.isim) {
                throw new Error(`${index + 1}. karakterin ismi eksik.`);
            }

            const blocks = {};

            (character.akislar || []).forEach((flow, flowIndex) => {
                if (!flow || flow.tetikleyici !== "olay_bayrak") {
                ...
                const firstChildId = compileBlockList(flow.bloklar || [], blocks, eventId);
            ...
            targets.push({
                ...
                costumes: [
                    {
                        ...
                        name: `${character.isim} Kostümü`,
                        bitmapResolution: 1,
                        md5ext: `${costume.assetId}.svg`,
                        dataFormat: "svg",
                        rotationCenterX: costume.width / 2,
                        rotationCenterY: costume.height / 2
                    }
                ],
                sounds: [],
                volume: 100,
                layerOrder: index + 1,
                visible: true,
                x: safeNumber(character.ilk_x, 0),
                y: safeNumber(character.ilk_y, 0),
                size: safeNumber(character.boyut, 100),
                direction: safeNumber(character.yon, 90),
                draggable: false,
    ...
    function createZip(model) {
        ...
        compiled.assets.forEach((content, filename) => {
            zip.file(filename, content);
        });

        return {
            zip,
            targetCount: compiled.project.targets.length
        };
    }

    function editorPlugin(targetCount) {
        const code = `(() => {
            const expectedTargets=${targetCount};
            
            // Inject CSS to hide the logo immediately
            const style = document.createElement('style');
            style.textContent = \`
                [class*="scratch-logo"], 
            ...
            const prepare = (force = false) => {
                const ready = force || document.querySelector('.blocklyBlockCanvas') || (window.vm && window.vm.runtime && window.vm.runtime.targets.length >= expectedTargets);
                const loader = document.querySelector('[class*="loader_background"]') || document.querySelector('[class*="loader_loader"]');
                if (ready && loader) {
                    loader.remove();
                    clearInterval(timer);
            ...
            timer = setInterval(() => prepare(false), 200);
            window.addEventListener('load', () => prepare(false));
            
            // Force unlock after 8 seconds under any condition to prevent freezing
    ...
    async function load(input) {
        ...
        loading.textContent = "Scratch editörü hazırlanıyor...";
        ...
        try {
            ...
            frame.onerror = () => {
                loading.hidden = false;
                loading.textContent = "Editör yüklenemedi.";
            };
    ...
    window.ScratchGame = Object.freeze({
        load,
        download
    });
